...
############ 서버가 켜질 때 바로 TTS 엔진을 로드하여 초기화 시간을 줄이도록 수정
# 전역 TTS 엔진 (서버 시작 시 한번만 로드)
_tts_engine = None

def get_tts_engine() -> SupertonicONNXTTS:
    """TTS 엔진 싱글톤"""
    global _tts_engine
    if _tts_engine is None:
        # 환경변수로 GPU 사용 여부 제어 (기본값: True)
        use_gpu = os.getenv("TTS_USE_GPU", "true").lower() == "true"
        _tts_engine = SupertonicONNXTTS(
...
def synthesize(
    ...
    metrics = {
        ...
        "rtf": round(inference_time / audio_duration, 2) if audio_duration > 0 else 0,  # Real-time Factor
    }
